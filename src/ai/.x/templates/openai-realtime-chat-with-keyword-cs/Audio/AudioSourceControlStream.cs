public class AudioSourceControlStream : Stream, IDisposable
{
    public AudioSourceControlStream(AudioSourceController controller)
    {
    ...
    private void CopyToBuffer(AudioDataAvailableEventArgs e)
    {
        lock (_bufferLock)
        {
            ...
            if (_bufferWritePos + bytesToCopy >= _buffer.Length)
            {
                ...
                Array.Copy(e.AudioData, 0, _buffer, _bufferWritePos, bytesToCopyBeforeWrap);
                bytesToCopy -= bytesToCopyBeforeWrap;
                _bufferWritePos = 0;
            }
            Array.Copy(e.AudioData, e.BytesRecorded - bytesToCopy, _buffer, _bufferWritePos, bytesToCopy);
