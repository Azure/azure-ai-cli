...
{
    public {ClassName}(string apiKey, string endpoint, string model, string instructions, FunctionFactory factory, MicrophoneAudioInputStream microphone, SpeakerAudioOutputStream speaker)
    {
        ...
        await foreach (var update in _session.ReceiveUpdatesAsync())
        {
            ...
            switch (update)
            {
                ...
                case ConversationResponseFinishedUpdate turnFinishedUpdate:
                    await HandleTurnFinished(turnFinishedUpdate);
                    break;
    ...
    private async Task HandleTurnFinished(ConversationResponseFinishedUpdate turnFinishedUpdate)
    {
