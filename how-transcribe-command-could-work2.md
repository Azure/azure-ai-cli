To ensure that the `ai speech transcribe` command correctly handles all the options specified in the SDE instructions, including turning diarization on/off and other configurations, the implementation needs to be carefully reviewed and updated. Below is a revised plan with detailed steps to correctly handle all specified options, including how to properly configure the request body parameters like diarization, locales, and channels.
  ...
  - Implement the `TranscribeCommandParser` class similar to `RecognizeCommandParser`:
    ...
    public class TranscribeCommandParser : CommandParser
    {
        ...
        private static INamedValueTokenParser[] transcribeCommandParsers = {
            ...
            new TrueFalseNamedValueTokenParser("--diarization", "diarization.config.enabled", "01"),
            new Any1ValueNamedValueTokenParser("--maxSpeakers", "diarization.config.maxSpeakers", "01"),
  ...
  - Create a new `TranscribeCommand` class similar to `RecognizeCommand`:
    ...
    public class TranscribeCommand : Command
    {
        ...
        private string GetDefinitionJson()
        {
            ...
            var diarizationEnabled = _values.GetOrDefault("diarization.config.enabled", false);
            var maxSpeakers = _values.GetOrDefault("diarization.config.maxSpeakers", 1);
        ...
        private void ProcessResponse(string jsonResponse)
        {
            ...
            _output!.EnsureOutputAll("transcription.result", responseObj);
            _output!.CheckOutput();
        }
